...
namespace :db do
  ...
  task dump: :environment do
    ...
    with_config do |app, host, db, user, pass|
      ...
      cmd       = "PGPASSWORD='#{pass}' pg_dump -F #{dump_fmt} -v -O -U '#{user}' -h '#{host}' -d '#{db}' -f '#{full_path}'"
    end
    ...
    task table: :environment do
      ...
      if table_name.present?
        ...
        with_config do |app, host, db, user, pass|
          ...
          cmd       = "PGPASSWORD='#{pass}' pg_dump -F #{dump_fmt} -v -O -U '#{user}' -h '#{host}' -d '#{db}' -t '#{table_name}' -f '#{full_path}'"
        end
    ...
    if pattern.present?
      ...
      with_config do |app, host, db, user, pass|
        ...
        when 1
          ...
          when 'p'
            cmd = "PGPASSWORD='#{pass}' psql -U '#{user}' -h '#{host}' -d '#{db}' -f '#{file}'"
          else
  ...
  def backup_directory(suffix = nil, create: false)
    backup_dir = File.join(*[Rails.root, 'db_backups', suffix].compact)
